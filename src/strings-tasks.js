...
function sumOfCodes(str) {
  ...
  return arrayStr.reduce((sum, char) => sum + char.charCodeAt(0), 0);
}
...
function findLongestWord(sentence) {
  const sortedWords = sentence.split(' ').sort((a, b) => b.length - a.length);
...
function extractNameFromTemplate(value) {
  const trimmedValue = value.replace('Hello, ', '').replace('!', '');
